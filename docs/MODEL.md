...
The inverse current calculation brackets a scalar root in $y=\ln(1+I/I_s)$ to avoid uncontrolled exponential iteration. Reverse breakdown, shunt leakage, high injection and self-heating are outside the compact model. The included case study is explicitly **SYNTHETIC DATA**; no experimental validation is claimed.
